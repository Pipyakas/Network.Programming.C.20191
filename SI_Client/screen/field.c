...
void field_draw(void *renderer) {
    ...
    font_render(info, 0, 0, 0, assets_bundle->fonts[2], w);

    /**
     * Calculating player's HP
     */
...
void field_update() {
    ...
    if (players[client_id]->health > last_health)
        last_health = players[client_id]->health; // restore, if pickup'd healthpack
